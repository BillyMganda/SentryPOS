...
{
    public partial class Sales_by_Item : Form
    {
        public Sales_by_Item()
        {
            InitializeComponent();
        }
        ...
        private void Sales_by_Item_Load(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(connectionstring))
                {
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT * FROM Sales WHERE Sub_Total <> '' AND Total <> '' ORDER BY ID DESC", con);
                    DataSet dst = new DataSet();
                    sda.Fill(dst, "Sales");
                    crystal.Load(@"C:\Users\Billy Isogol\Desktop\C#\SentryPOS\SentryPOS\CrystalReports\SalesByItem.rpt");
